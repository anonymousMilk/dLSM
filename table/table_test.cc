...
// Special test for a block with no restart entries.  The C++ dLSM
// code never generates such blocks, but the Java version of dLSM
// seems to.
...
TEST_F(Harness, RandomizedLongDB) {
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    std::string value;
    char name[100];
    std::snprintf(name, sizeof(name), "dLSM.num-files-at-level%d", level);
