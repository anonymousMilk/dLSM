...
TEST_F(EnvPosixTest, TestOpenOnRead) {
  ...
  // open-on-read behavior of POSIX Env dLSM::RandomAccessFile.
...
TEST_F(EnvPosixTest, TestCloseOnExecSequentialFile) {
  ...
  ASSERT_dLSM_OK(WriteStringToFile(env_, "0123456789", file_path));

  dLSM::SequentialFile* file = nullptr;
  ASSERT_dLSM_OK(env_->NewSequentialFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;

  ASSERT_dLSM_OK(env_->RemoveFile(file_path));
}
...
TEST_F(EnvPosixTest, TestCloseOnExecRandomAccessFile) {
  ...
  // mmap region.
  dLSM::RandomAccessFile* mmapped_files[kReadOnlyFileLimit] = {nullptr};
  for (int i = 0; i < kReadOnlyFileLimit; i++) {
    ASSERT_dLSM_OK(env_->NewRandomAccessFile(file_path, &mmapped_files[i]));
  }

  dLSM::RandomAccessFile* file = nullptr;
  ASSERT_dLSM_OK(env_->NewRandomAccessFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecWritableFile) {
  ...
  dLSM::WritableFile* file = nullptr;
  ASSERT_dLSM_OK(env_->NewWritableFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecAppendableFile) {
  ...
  dLSM::WritableFile* file = nullptr;
  ASSERT_dLSM_OK(env_->NewAppendableFile(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
...
TEST_F(EnvPosixTest, TestCloseOnExecLockFile) {
  ...
  dLSM::FileLock* lock = nullptr;
  ASSERT_dLSM_OK(env_->LockFile(file_path, &lock));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
...
TEST_F(EnvPosixTest, TestCloseOnExecLogger) {
  ...
  dLSM::Logger* file = nullptr;
  ASSERT_dLSM_OK(env_->NewLogger(file_path, &file));
  CheckCloseOnExecDoesNotLeakFDs(open_fds);
  delete file;
