...
class WindowsSequentialFile : public SequentialFile {
 public:
  ...
  Status Read(size_t n, Slice* result, char* scratch) override {
    DWORD bytes_read;
    // DWORD is 32-bit, but size_t could technically be larger. However dLSM
    // files are limited to dLSM::Options::max_file_size which is clamped to
    // 1<<30 or 1 GiB.
...
class WindowsEnv : public Env {
 public:
  ...
  Status GetTestDirectory(std::string* result) override {
    ...
    std::stringstream ss;
    ss << tmp_path << "dLSMtest-" << std::this_thread::get_id();
    *result = ss.str();
