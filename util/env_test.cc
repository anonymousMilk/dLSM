...
TEST_F(EnvTest, ReadWrite) {
  ...
  ASSERT_dLSM_OK(env_->NewWritableFile(test_file_name, &writable_file));
  ...
  while (data.size() < kDataSize) {
    ...
    ASSERT_dLSM_OK(writable_file->Append(r));
    data += r;
    if (rnd.OneIn(10)) {
  ...
  ASSERT_dLSM_OK(env_->NewSequentialFile(test_file_name, &sequential_file));
  ...
  while (read_result.size() < data.size()) {
    ...
    Slice read;
    ASSERT_dLSM_OK(sequential_file->Read(len, &read, &scratch[0]));
    if (len > 0) {
...
TEST_F(EnvTest, ReopenWritableFile) {
  ...
  ASSERT_dLSM_OK(writable_file->Append(data));
  ASSERT_dLSM_OK(writable_file->Close());
  delete writable_file;
  ...
  ASSERT_dLSM_OK(ReadFileToString(env_, test_file_name, &data));
...
TEST_F(EnvTest, ReopenAppendableFile) {
  ...
  ASSERT_dLSM_OK(env_->NewAppendableFile(test_file_name, &appendable_file));
  std::string data("hello world!");
  ASSERT_dLSM_OK(appendable_file->Append(data));
  ASSERT_dLSM_OK(appendable_file->Close());
