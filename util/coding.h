...
extern void PutFixed16(Slice* dst, uint32_t value);
extern void PutFixed32(Slice* dst, uint32_t value);
extern void PutFixed64(Slice* dst, uint64_t value);
extern void PutVarint32(std::string* dst, uint32_t value);
extern void PutVarint32(Slice* dst, uint32_t value);
...
extern void PutVarint64(Slice* dst, uint64_t value);
...
// Pull the last 8 bits and cast it to a character
inline void PutFixed16(Slice* dst, uint16_t value) {
  if (port::kLittleEndian) {
    dst->append(const_cast<const char*>(reinterpret_cast<char*>(&value)),
                sizeof(value));
  } else {
...
inline void PutFixed32(Slice* dst, uint32_t value) {
  if (port::kLittleEndian) {
    dst->append(const_cast<const char*>(reinterpret_cast<char*>(&value)),
                sizeof(value));
  } else {
...
inline void PutFixed64(Slice* dst, uint64_t value) {
  if (port::kLittleEndian) {
    dst->append(const_cast<const char*>(reinterpret_cast<char*>(&value)),
                sizeof(value));
  } else {
...
inline void PutVarint32(Slice* dst, uint32_t v) {
  char buf[5];
  char* ptr = EncodeVarint32(buf, v);
  dst->append(buf, static_cast<size_t>(ptr - buf));
}
...
inline void PutVarint64(Slice* dst, uint64_t v) {
  char buf[kMaxVarint64Length];
  char* ptr = EncodeVarint64(buf, v);
  dst->append(buf, static_cast<size_t>(ptr - buf));
}
