// Copyright 2014 The dLSM Authors. All rights reserved.
 ...
 public:
  ...
  void DeleteAllData() {
    ...
    for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
      ASSERT_dLSM_OK(db_->Delete(WriteOptions(), iter->key()));
    }
  ...
  void ResetDBState(ResetMethod reset_method) {
    switch (reset_method) {
      case RESET_DROP_UNSYNCED_DATA:
        ASSERT_dLSM_OK(env_->DropUnsyncedFileData());
        break;
      case RESET_DELETE_UNSYNCED_FILES:
        ASSERT_dLSM_OK(env_->RemoveFilesCreatedAfterLastDirSync());
        break;
      default:
        assert(false);
    }
