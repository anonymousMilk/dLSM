...
class RecoveryTest : public testing::Test {
 public:
  ...
  void Open(Options* options = nullptr) {
    ASSERT_dLSM_OK(OpenWithStatus(options));
    ASSERT_EQ(1, NumLogs());
  }
  ...
  size_t RemoveLogFiles() {
    ...
    for (size_t i = 0; i < logs.size(); i++) {
      EXPECT_dLSM_OK(env_->RemoveFile(LogName(logs[i]))) << LogName(logs[i]);
    }
    return logs.size();
  }

  void RemoveManifestFile() {
    ASSERT_dLSM_OK(env_->RemoveFile(ManifestFileName()));
  }
  ...
  uint64_t FileSize(const std::string& fname) {
    uint64_t result;
    EXPECT_dLSM_OK(env_->GetFileSize(fname, &result)) << fname;
    return result;
  }
  ...
  void MakeLogFile(uint64_t lognum, SequenceNumber seq, Slice key, Slice val) {
    ...
    WritableFile* file;
    ASSERT_dLSM_OK(env_->NewWritableFile(fname, &file));
    log::Writer writer(file);
    WriteBatch batch;
    batch.Put(key, val);
    WriteBatchInternal::SetSequence(&batch, seq);
    ASSERT_dLSM_OK(writer.AddRecord(WriteBatchInternal::Contents(&batch)));
    ASSERT_dLSM_OK(file->Flush());
    delete file;
  }
...
TEST_F(RecoveryTest, LargeManifestCompacted) {
  ...
  // Pad with zeroes to make manifest file very big.
  {
    ...
    WritableFile* file;
    ASSERT_dLSM_OK(env()->NewAppendableFile(old_manifest, &file));
    std::string zeroes(3 * 1048576 - static_cast<size_t>(len), 0);
    ASSERT_dLSM_OK(file->Append(zeroes));
    ASSERT_dLSM_OK(file->Flush());
    delete file;
  }
