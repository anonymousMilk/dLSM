...
class DBTest : public testing::Test {
 public:
  ...
  void Reopen(Options* options = nullptr) {
    ASSERT_dLSM_OK(TryReopen(options));
  }
  ...
  int NumTableFilesAtLevel(int level) {
    ...
    EXPECT_TRUE(db_->GetProperty(
        "dLSM.num-files-at-level" + NumberToString(level), &property));
  ...
  std::string DumpSSTableList() {
    std::string property;
    db_->GetProperty("dLSM.sstables", &property);
    return property;
  }
  ...
  bool DeleteAnSSTFile() {
    ...
    EXPECT_dLSM_OK(env_->GetChildren(dbname_, &filenames));
    uint64_t number;
    FileType type;
    for (size_t i = 0; i < filenames.size(); i++) {
      if (ParseFileName(filenames[i], &number, &type) && type == kTableFile) {
        EXPECT_dLSM_OK(env_->RemoveFile(TableFileName(dbname_, number)));
        return true;
      }
    }
    return false;
  ...
  int RenameLDBToSST() {
    ...
    for (size_t i = 0; i < filenames.size(); i++) {
      if (ParseFileName(filenames[i], &number, &type) && type == kTableFile) {
        ...
        EXPECT_dLSM_OK(env_->RenameFile(from, to));
        files_renamed++;
      }
    }
...
TEST_F(DBTest, EmptyKey) {
  do {
    ASSERT_dLSM_OK(Put("", "v1"));
    ASSERT_EQ("v1", Get(""));
    ASSERT_dLSM_OK(Put("", "v2"));
    ASSERT_EQ("v2", Get(""));
  } while (ChangeOptions());
}
...
TEST_F(DBTest, EmptyValue) {
  do {
    ASSERT_dLSM_OK(Put("key", "v1"));
    ASSERT_EQ("v1", Get("key"));
    ASSERT_dLSM_OK(Put("key", ""));
    ASSERT_EQ("", Get("key"));
    ASSERT_dLSM_OK(Put("key", "v2"));
...
TEST_F(DBTest, ReadWrite) {
  do {
    ASSERT_dLSM_OK(Put("foo", "v1"));
    ASSERT_EQ("v1", Get("foo"));
    ASSERT_dLSM_OK(Put("bar", "v2"));
    ASSERT_dLSM_OK(Put("foo", "v3"));
...
TEST_F(DBTest, PutDeleteGet) {
  do {
    ASSERT_dLSM_OK(db_->Put(WriteOptions(), "foo", "v1"));
    ASSERT_EQ("v1", Get("foo"));
    ASSERT_dLSM_OK(db_->Put(WriteOptions(), "foo", "v2"));
    ASSERT_EQ("v2", Get("foo"));
    ASSERT_dLSM_OK(db_->Delete(WriteOptions(), "foo"));
    ASSERT_EQ("NOT_FOUND", Get("foo"));
  } while (ChangeOptions());
}
...
TEST_F(DBTest, GetMemUsage) {
  do {
    ASSERT_dLSM_OK(Put("foo", "v1"));
    std::string val;
    ASSERT_TRUE(db_->GetProperty("dLSM.approximate-memory-usage", &val));
...
TEST_F(DBTest, GetPicksCorrectFile) {
  do {
    ...
    ASSERT_dLSM_OK(Put("a", "va"));
    Compact("a", "b");
    ASSERT_dLSM_OK(Put("x", "vx"));
    Compact("x", "y");
    ASSERT_dLSM_OK(Put("f", "vf"));
    Compact("f", "g");
    ASSERT_EQ("va", Get("a"));
    ASSERT_EQ("vf", Get("f"));
    ASSERT_EQ("vx", Get("x"));
...
TEST_F(DBTest, IterMulti) {
  ASSERT_dLSM_OK(Put("a", "va"));
  ASSERT_dLSM_OK(Put("b", "vb"));
  ASSERT_dLSM_OK(Put("c", "vc"));
  Iterator* iter = db_->NewIterator(ReadOptions());
  ...
  ASSERT_dLSM_OK(Put("a", "va2"));
  ASSERT_dLSM_OK(Put("a2", "va3"));
  ASSERT_dLSM_OK(Put("b", "vb2"));
  ASSERT_dLSM_OK(Put("c", "vc2"));
  ASSERT_dLSM_OK(Delete("b"));
  iter->SeekToFirst();
  ASSERT_EQ(IterStatus(iter), "a->va");
  iter->Next();
  ASSERT_EQ(IterStatus(iter), "b->vb");
...
TEST_F(DBTest, IterSmallAndLargeMix) {
  ASSERT_dLSM_OK(Put("a", "va"));
  ASSERT_dLSM_OK(Put("b", std::string(100000, 'b')));
  ASSERT_dLSM_OK(Put("c", "vc"));
  ASSERT_dLSM_OK(Put("d", std::string(100000, 'd')));
  ASSERT_dLSM_OK(Put("e", std::string(100000, 'e')));
...
TEST_F(DBTest, MinorCompactionsHappen) {
  ...
  for (int i = 0; i < N; i++) {
    ASSERT_dLSM_OK(Put(Key(i), Key(i) + std::string(1000, 'v')));
  }
...
TEST_F(DBTest, RecoverWithLargeLog) {
  {
    Options options = CurrentOptions();
    Reopen(&options);
    ASSERT_dLSM_OK(Put("big1", std::string(200000, '1')));
    ASSERT_dLSM_OK(Put("big2", std::string(200000, '2')));
    ASSERT_dLSM_OK(Put("small3", std::string(10, '3')));
    ASSERT_dLSM_OK(Put("small4", std::string(10, '4')));
    ASSERT_EQ(NumTableFilesAtLevel(0), 0);
  }
...
TEST_F(DBTest, CompactionsGenerateMultipleFiles) {
  ...
  for (int i = 0; i < 80; i++) {
    ...
    ASSERT_dLSM_OK(Put(Key(i), values[i]));
  }
...
TEST_F(DBTest, ApproximateSizes) {
  do {
    ...
    Random rnd(301);
    for (int i = 0; i < N; i++) {
      ASSERT_dLSM_OK(Put(Key(i), RandomString(&rnd, S1)));
    }
...
TEST_F(DBTest, ApproximateSizes_MixOfSmallAndLarge) {
  do {
    ...
    ASSERT_dLSM_OK(Put(Key(0), RandomString(&rnd, 10000)));
    ASSERT_dLSM_OK(Put(Key(1), RandomString(&rnd, 10000)));
    ASSERT_dLSM_OK(Put(Key(2), big1));
    ASSERT_dLSM_OK(Put(Key(3), RandomString(&rnd, 10000)));
    ASSERT_dLSM_OK(Put(Key(4), big1));
    ASSERT_dLSM_OK(Put(Key(5), RandomString(&rnd, 10000)));
    ASSERT_dLSM_OK(Put(Key(6), RandomString(&rnd, 300000)));
    ASSERT_dLSM_OK(Put(Key(7), RandomString(&rnd, 10000)));

    if (options.reuse_logs) {
      // Need to force a memtable compaction since recovery does not do so.
      ASSERT_dLSM_OK(dbfull()->TEST_CompactMemTable());
    }
...
TEST_F(DBTest, DeletionMarkers1) {
  ...
  ASSERT_dLSM_OK(dbfull()->TEST_CompactMemTable());  // Moves to level last-2
  ASSERT_EQ(AllEntriesFor("foo"), "[ v2, DEL, v1 ]");
  Slice z("z");
...
TEST_F(DBTest, OverlapInLevel0) {
  do {
    ...
    // 0.
    ASSERT_dLSM_OK(Put("100", "v100"));
    ASSERT_dLSM_OK(Put("999", "v999"));
    dbfull()->TEST_CompactMemTable();
    ASSERT_dLSM_OK(Delete("100"));
    ASSERT_dLSM_OK(Delete("999"));
    dbfull()->TEST_CompactMemTable();
    ...
    ASSERT_dLSM_OK(Put("300", "v300"));
    ASSERT_dLSM_OK(Put("500", "v500"));
    dbfull()->TEST_CompactMemTable();
    ASSERT_dLSM_OK(Put("200", "v200"));
    ASSERT_dLSM_OK(Put("600", "v600"));
    ASSERT_dLSM_OK(Put("900", "v900"));
    dbfull()->TEST_CompactMemTable();
...
TEST_F(DBTest, L0_CompactionBug_Issue44_a) {
  Reopen();
  ASSERT_dLSM_OK(Put("b", "v"));
  Reopen();
  ASSERT_dLSM_OK(Delete("b"));
  ASSERT_dLSM_OK(Delete("a"));
  Reopen();
  ASSERT_dLSM_OK(Delete("a"));
  Reopen();
  ASSERT_dLSM_OK(Put("a", "v"));
  Reopen();
  Reopen();
...
TEST_F(DBTest, Fflush_Issue474) {
  ...
  for (int i = 0; i < kNum; i++) {
    std::fflush(nullptr);
    ASSERT_dLSM_OK(Put(RandomKey(&rnd), RandomString(&rnd, 100)));
  }
}
...
TEST_F(DBTest, ComparatorCheck) {
  class NewComparator : public Comparator {
   public:
    const char* Name() const override { return "dLSM.NewComparator"; }
...
TEST_F(DBTest, CustomComparator) {
  ...
  ASSERT_dLSM_OK(Put("[10]", "ten"));
  ASSERT_dLSM_OK(Put("[0x14]", "twenty"));
  for (int i = 0; i < 2; i++) {
...
TEST_F(DBTest, DestroyEmptyDir) {
  ...
  ASSERT_dLSM_OK(env.CreateDir(dbname));
  ASSERT_TRUE(env.FileExists(dbname));
  std::vector<std::string> children;
  ASSERT_dLSM_OK(env.GetChildren(dbname, &children));
  // The stock Env's do not filter out '.' and '..' special files.
  ASSERT_EQ(2, children.size());
  ASSERT_dLSM_OK(DestroyDB(dbname, opts));
  ASSERT_TRUE(!env.FileExists(dbname));
...
TEST_F(DBTest, DestroyOpenDB) {
  ...
  opts.create_if_missing = true;
  DB* db = nullptr;
  ASSERT_dLSM_OK(DB::Open(opts, dbname, &db));
  ASSERT_TRUE(db != nullptr);
  ...
  ASSERT_dLSM_OK(DestroyDB(dbname, Options()));
  ASSERT_TRUE(!env_->FileExists(dbname));
}
...
TEST_F(DBTest, WriteSyncError) {
  ...
  WriteOptions w;
  ASSERT_dLSM_OK(db_->Put(w, "k1", "v1"));
  ASSERT_EQ("v1", Get("k1"));
...
TEST_F(DBTest, BloomFilter) {
  ...
  const int N = 10000;
  for (int i = 0; i < N; i++) {
    ASSERT_dLSM_OK(Put(Key(i), Key(i)));
  }
  Compact("a", "z");
...
static void MTThreadBody(void* arg) {
  ...
  while (!t->state->stop.load(std::memory_order_acquire)) {
    ...
    if (rnd.OneIn(2)) {
      ...
      ASSERT_dLSM_OK(db->Put(WriteOptions(), Slice(keybuf), Slice(valbuf)));
    } else {
  ...
  do {
    ...
    for (int step = 0; step < N; step++) {
      ...
      if (p < 45) {  // Put
        ...
        ASSERT_dLSM_OK(model.Put(WriteOptions(), k, v));
        ASSERT_dLSM_OK(db_->Put(WriteOptions(), k, v));

      } else if (p < 90) {  // Delete
        k = RandomKey(&rnd);
        ASSERT_dLSM_OK(model.Delete(WriteOptions(), k));
        ASSERT_dLSM_OK(db_->Delete(WriteOptions(), k));
        ...
        ASSERT_dLSM_OK(model.Write(WriteOptions(), &b));
        ASSERT_dLSM_OK(db_->Write(WriteOptions(), &b));
      }
...
static void BM_LogAndApply(benchmark::State& state) {
  ...
  std::string dbname = testing::TempDir() + "dLSM_test_benchmark";
  DestroyDB(dbname, Options());
  ...
  bool save_manifest;
  ASSERT_dLSM_OK(vset.Recover(&save_manifest));
  VersionEdit vbase;
  uint64_t fnum = 1;
  ...
  ASSERT_dLSM_OK(vset.LogAndApply(&vbase, 0));
